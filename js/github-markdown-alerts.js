(function () {
  ...
  const makeAlert = (blockquote) => {
    ...
    const titleText = key[0] + key.slice(1).toLowerCase();
    const titleIcon = map[key].icon;
    const title = makeHTML('<p class="markdown-alert-title"></p>');
    ...
    if (result[2] === "") {
      ...
      firstChild.innerHTML = firstChild.innerHTML.slice(
        "![]".length + key.length + result[2].length
      );
    }
    const div = document.createElement("div");
  ...
  //       Array.from(mutation.addedNodes).forEach(makeAlert);
  //     }
  //   });
  // });
  // observer.observe(document, { childList: true, subtree: true });
  document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll("blockquote").forEach(makeAlert);
  });
})();
